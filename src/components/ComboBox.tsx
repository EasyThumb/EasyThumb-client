...
type Props = {
    options: string[] | Accessor<string[]>;
    placeholder?: string;
    onSelect?: (value: string) => void;
    height?: number;
};

export default function ComboBox({ options, placeholder = 'Select option...', onSelect, height = 300 }: Props) {
    ...
    const filtered = createMemo(() => resolvedOptions().filter((o) => o.toLowerCase().includes(search().toLowerCase())));
    ...
    createEffect(() => {
        if (open() && selected()) {
            const index = filtered().findIndex((o) => o === selected());
            if (index !== -1) {
    ...
    return (
        <div class="combo-box relative w-64">
            ...
            >
                <span class="text-gray-700 text-sm truncate">{selected() || placeholder}</span>
